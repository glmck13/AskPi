...
## Use case: Connecting via Alexa
The AlexaPiHome project shows how to build an Alexa app (which I name "Charlie") that issues commands to a Raspberry Pi on your local network listening to an HTTPS port opened to the Internet.  The Pi hosting the Alexa-facing server then forwards commands to the AskPi server by invoking the fifo.cgi script.  fifo.cgi simply injects the text driectives it receives into the askpi.fifo pipe, which the askpi.sh client reads from.  
