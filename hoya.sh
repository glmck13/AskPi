...
game)
	...
	echo -n $(curl -s http://www.guhoyas.com/sports/mens-basketball/schedule | grep -A$comptail sidearm-schedule-game-completed | grep -A$hidetail Hide | tail -$hidetail | recode -f html..ascii | sed -e "s/<[^>]*>//g" -e "s/*//g" -e "s/ vs/versus/" -e "s/Live//" -e "s/Stats//" -e "s/Tickets//" -e "s/Preview//" | tr -c "[:print:]" " ")
	print "</p>"
	;;
